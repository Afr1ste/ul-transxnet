...
\graphicspath{{./}}
\newcommand{\suppsection}[1]{\Needspace{0.35\textheight}\section*{#1}}

\begin{document}

\begin{center}
{\Large\bfseries Supplementary Material for ``UltraDrill: A Distillation and Optimization Framework for Edge Ultrasound Lesion Classification''\par}
\end{center}
\vspace{1em}

\suppsection{S1. Result Provenance}
...
\begin{table}[H]
    ...
    \begin{tabular}{lll}
        ...
        UltraDrill localization stress & Strict box-noise, detector-ROI, and full-image report & Teachers and edge students under matched inputs \\
        Edge export/runtime & Strict TN5000 mobile export and two-device Android reports & Re-exported artifacts \\
...
\suppsection{S2. Full Main Benchmark Context}

\begin{table}[H]
    \centering
    \small
    \caption{Dataset-specific benchmark context under the fixed reporting protocol. Values are mean $\pm$ standard deviation over TN5000 seeds or BUSI/AUL training folds evaluated on the fixed held-out test sets.}
...
\suppsection{S3. TransXNet-family Design-space Details}
...
\suppsection{S4. ROI Robustness}

\begin{table}[H]
    \centering
    \small
    \caption{TN5000 teacher-student localization-stress summary. Drop@20\% is GT-ROI AUC minus 20\% box-noise AUC.}
    \label{tab:s_teacher_student_robustness}
    \begin{adjustbox}{max width=\columnwidth}
    \begin{tabular}{lccccc}
        \toprule
        Model & GT ROI & 20\% noise & Drop@20\% & Detector ROI & Full image \\
        \midrule
...
\begin{table}[H]
    \centering
    \small
    \caption{Teacher-side TN5000 AUC robustness context across the four models shown in the main robustness figure under the fixed reporting protocol.}
...
\suppsection{S5. Android Runtime Details}

\begin{table}[H]
    \centering
    \small
    \caption{Two-device Android runtime sweep for exported EfficientFormer artifacts. AUC is computed from exported predictions under the fixed TN5000 test protocol. Hot latency is end-to-end total latency in milliseconds.}
...
\begin{table}[H]
    \centering
    \small
    \caption{Exported mobile-student comparison under CPU default runtime. Offline metrics are from the strict export summary; Android AUC and latency are from the two-device headless run.}
...
\suppsection{S6. Multi-organ Joint and LODO Details}
...
\suppsection{S7. BUSI Duplicate Audit}
...
\begin{table}[H]
    \centering
    \small
    \caption{Descriptive BUSI duplicate and near-duplicate audit.}
    ...
    \begin{tabular}{lcccl}
        ...
        \midrule
        Strict ($\leq 2$) & 1 & 88 & 26 & \texttt{busi\_duplicate\_audit\_strict.csv} \\
        Relaxed ($\leq 5$) & 1 & 141 & 41 & \texttt{busi\_duplicate\_audit\_relaxed.csv} \\
        \bottomrule
    \end{tabular}
    \end{adjustbox}
\end{table}

\suppsection{S8. Removed Legacy Tables}
