...
LATEST_PTR = PROJECT_ROOT / "eval_reports" / "aul_compare_extra4models_latest.txt"

EXTRA4_CONFIGS = [
    dict(
        name="DENSENET121_aul_bin_autoCW_exp030",
    ...
    dict(
        name="CONVNEXTT_aul_bin_autoCW_exp030",
    ...
    dict(
        name="REPVITM11_aul_bin_autoCW_exp030",
    ...
    dict(
        name="EFFFORMERL1_aul_bin_autoCW_exp030",
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="AUL extra-4 baseline comparison on 5 folds."
    )
    p.add_argument("--output-root", default="aul_roi_runs_compare_extra4models_5fold")
    p.add_argument("--log-root", default="aul_compare_extra4models_5fold_logs")
...
def main() -> int:
    ...
    if bool(args.skip_if_complete):
        ...
        if prev is not None and is_complete(prev, expected_runs):
            ...
            print(f"[SKIP] Existing complete AUL extra4 compare found: {prev}")
            return 0
    ...
    try:
        sys.argv = [
            ...
            base.LOG_ROOT,
            "--only-configs",
            ",".join(cfg["name"] for cfg in selected_configs),
            "--continue-on-error",
