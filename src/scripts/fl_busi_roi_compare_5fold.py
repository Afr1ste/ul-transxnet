...
class Config:
    ...
    param_tag: str = "OURS_autoCW_exp030_f0"
    best_model_name: str = "best_model_busi_roi.pth"
    final_model_name: str = "last_model_busi_roi.pth"
...
class BUSIVOCRoiDataset(Dataset):
    def __init__(self, root_dir: str, split: str, transform=None,
        ...
        print(f"[BUSI-ROI] split={split}, samples={len(self.samples)}, label_counts={self.label_counts}")
    ...
    def _load_label_manifest(self) -> Dict[str, int]:
        ...
        with open(self.label_manifest, 'r', encoding='utf-8-sig', newline='') as f:
            ...
            for row in reader:
                stem = str(row.get('new_stem', '')).strip()
                fname = str(row.get('new_filename', '')).strip()
                label = int(row['label'])
                if stem:
                    label_map[stem] = label
                if fname:
                    label_map[fname] = label
                    label_map[Path(fname).stem] = label
        return label_map
...
def prepare_datasets(seed: int = 17):
    ...
    datasets = {
        'train': BUSIVOCRoiDataset(Config.data_root, Config.train_split, train_transform,
        ...
        'val': BUSIVOCRoiDataset(Config.data_root, Config.val_split, eval_transform,
                                 use_roi_crop=Config.use_roi_crop,
                                 bbox_expand_ratio=Config.bbox_expand_ratio,
                                 min_crop_size=Config.min_crop_size,
                                 use_whole_image_fallback=Config.use_whole_image_fallback),
        'test': BUSIVOCRoiDataset(Config.data_root, Config.test_split, eval_transform,
                                  use_roi_crop=Config.use_roi_crop,
                                  bbox_expand_ratio=Config.bbox_expand_ratio,
                                  min_crop_size=Config.min_crop_size,
                                  use_whole_image_fallback=Config.use_whole_image_fallback),
...
def build_class_weights(train_dataset: BUSIVOCRoiDataset, device: torch.device):
...
def save_summary(history: Dict, val_metrics: Dict, test_metrics: Dict,
    ...
    with open(txt_path, 'w', encoding='utf-8') as f:
        f.write('BUSI ROI Classification Summary\n')
        f.write('=' * 70 + '\n')
...
def main():
    ...
    print('=' * 80)
    print('BUSI ROI Training Script (Unified 5-fold)')
    print('=' * 80)
