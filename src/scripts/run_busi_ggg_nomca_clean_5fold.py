...
LATEST_PTR = PROJECT_ROOT / "eval_reports" / "busi_ggg_nomca_clean_latest.txt"


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="BUSI clean 5-fold TransXNet-GGG without MCA.")
    p.add_argument("--output-root", default="busi_roi_runs_ggg_nomca_clean_5fold_safe")
    p.add_argument("--log-root", default="busi_ggg_nomca_clean_5fold_safe_logs")
...
def main() -> None:
    ...
    if bool(args.skip_if_complete):
        ...
        if prev is not None and is_complete(prev, expected_runs):
            ...
            print(f"[SKIP] Existing complete BUSI no-MCA run found: {prev}")
            return

    cfg = dict(base.COMMON)
    cfg.update(
        {
            "name": "TRANSXNETGGG_NOMCA_busi_clean",
    ...
    print("=" * 100)
    print("BUSI clean GGG without MCA")
