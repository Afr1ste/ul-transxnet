...
LATEST_PTR = PROJECT_ROOT / "eval_reports" / "busi_p1_core_module_clean_latest.txt"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="BUSI clean P1 core module ablation.")
    p.add_argument("--output-root", default="busi_roi_runs_p1_core_module_clean_5fold")
    p.add_argument("--log-root", default="busi_p1_core_module_clean_5fold_logs")
...
def resolve_backbones(raw: str) -> list[dict]:
    ...
    if not selected:
        raise ValueError(f"No valid P1 BUSI backbones selected from: {raw}")
    return selected
...
def main() -> int:
    ...
    if bool(args.skip_if_complete):
        ...
        if prev is not None and is_complete(prev, expected_runs):
            ...
            print(f"[SKIP] Existing complete BUSI P1 run found: {prev}")
            return 0
    ...
    for cfg in selected:
        ...
        item.update(
            {
                "name": f"{cfg['key'].upper()}_busi_p1_clean",
    ...
    print("=" * 100)
    print("BUSI P1 clean core module ablation")
