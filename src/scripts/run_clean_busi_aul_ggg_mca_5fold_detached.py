...
MASTER_LOG_DIR = PROJECT_ROOT / "clean_dataset_ggg_mca_run_logs"
MASTER_LOG = MASTER_LOG_DIR / f"clean_busi_aul_ggg_mca_5fold_detached_{RUN_TS}.log"
STATUS_JSON = MASTER_LOG_DIR / f"clean_busi_aul_ggg_mca_5fold_detached_{RUN_TS}.status.json"
LATEST_STATUS = MASTER_LOG_DIR / "clean_busi_aul_ggg_mca_5fold_detached_latest.status.json"
...
COMMANDS = [
    {
        ...
        "args": [
            ...
            "--only-backbones", "transxnetggg",
            "--output-root", "busi_roi_runs_ggg_mca_clean_5fold_safe",
            "--log-root", "busi_ggg_mca_clean_5fold_safe_logs",
            "--skip-if-complete", "0",
            "--continue-on-error", "0",
        ],
    },
    {
        ...
        "args": [
            ...
            "--only-backbones", "transxnetggg",
            "--output-root", "aul_roi_runs_ggg_mca_clean_5fold_safe",
            "--log-root", "aul_ggg_mca_clean_5fold_safe_logs",
            "--skip-if-complete", "0",
            "--continue-on-error", "0",
        ],
    },
...
def main() -> int:
    ...
    if not PYTHON_EXE.exists():
        log(f"ERROR missing python exe: {PYTHON_EXE}")
        write_status({"state": "failed", "pid": os.getpid(), "error": f"missing python exe: {PYTHON_EXE}"})
        return 2
