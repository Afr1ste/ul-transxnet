...
# -*- coding: utf-8 -*-
"""
AUL ROI Binary：5 models x 5 folds 稳定版批跑脚本
...
TRAIN_SCRIPT = PROJECT_ROOT / "fl_aul_roi_binary_compare_5fold.py"
OUTPUT_ROOT = "aul_roi_runs_compare_5models_5fold"
LOG_ROOT = "aul_roi_compare_5models_5fold_logs"
DATA_ROOT = PROJECT_ROOT / "aul" / "aul_voc_roi_v1"
...
BASE_CONFIGS = [
    dict(
        name="OURS_aul_bin_autoCW_exp030",
        display_name="Ours",
        model_family="custom",
    ...
    dict(
        name="RESNET50_aul_bin_autoCW_exp030",
    ...
    dict(
        name="EFFB0_aul_bin_autoCW_exp030",
    ...
    dict(
        name="MBV3_aul_bin_autoCW_exp030",
    ...
    dict(
        name="SWINT_aul_bin_autoCW_exp030",
        display_name="Swin-T",
        model_family="timm",
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument('--log-root', default=LOG_ROOT)
    p.add_argument('--protocol-tag', default='aul_bin')
    p.add_argument('--only-configs', default='')
    p.add_argument('--continue-on-error', type=int, default=int(CONTINUE_ON_ERROR))
    return p.parse_args()


def apply_args(args: argparse.Namespace):
    global DATA_ROOT, OUTPUT_ROOT, LOG_ROOT, CONTINUE_ON_ERROR, EXPERIMENTS
    DATA_ROOT = Path(args.data_root)
    OUTPUT_ROOT = args.output_root
    LOG_ROOT = args.log_root
    CONTINUE_ON_ERROR = bool(args.continue_on_error)
    exps = build_experiments()
    if str(args.only_configs).strip():
        allow = {x.strip() for x in str(args.only_configs).split(',') if x.strip()}
        exps = [e for e in exps if e['base_name'] in allow or e['display_name'] in allow or e['name'] in allow]
    EXPERIMENTS = exps
...
def main():
    ...
    print('=' * 100)
    print('AUL ROI Binary：5 models x 5 folds 稳定版批跑脚本')
    print('=' * 100)
    ...
    readme = log_dir / 'README_AUL_BINARY_5MODELS_5FOLD.txt'
    readme.write_text(
        '\n'.join([
            'AUL ROI Binary compare: 5 models x 5 folds',
