...
LATEST_PTR = PROJECT_ROOT / "eval_reports" / "tn5000_compare_extra4models_latest.txt"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="TN5000 extra-4 baseline comparison on 3 seeds."
    )
    p.add_argument("--output-root", default="tn5000_roi_runs_compare_extra4models_3seed")
    p.add_argument("--log-root", default="tn5000_compare_extra4models_3seed_logs")
...
def main() -> int:
    ...
    expected_runs = len(selected_configs) * len(base.SEEDS)
    ...
    if bool(args.skip_if_complete):
        ...
        if prev is not None and is_complete(prev, expected_runs):
            ...
            print(f"[SKIP] Existing complete TN5000 extra4 compare found: {prev}")
            return 0
    ...
    print("=" * 100)
    print("TN5000 extra4 comparison")
