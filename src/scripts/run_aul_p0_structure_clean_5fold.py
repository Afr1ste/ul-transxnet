...
LATEST_PTR = PROJECT_ROOT / "eval_reports" / "aul_p0_structure_clean_latest.txt"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="AUL clean P0 structure ablation.")
    p.add_argument("--output-root", default="aul_roi_runs_p0_structure_clean_5fold")
    p.add_argument("--log-root", default="aul_p0_structure_clean_5fold_logs")
...
def resolve_backbones(raw: str) -> list[dict]:
    ...
    if not selected:
        raise ValueError(f"No valid P0 AUL backbones selected from: {raw}")
    return selected
...
def main() -> int:
    ...
    if bool(args.skip_if_complete):
        ...
        if prev is not None and is_complete(prev, expected_runs):
            ...
            print(f"[SKIP] Existing complete AUL P0 run found: {prev}")
            return 0
    ...
    for cfg in selected:
        ...
        item.update(
            {
                "name": f"{cfg['key'].upper()}_aul_p0_clean",
    ...
    print("=" * 100)
    print("AUL P0 clean structure ablation")
